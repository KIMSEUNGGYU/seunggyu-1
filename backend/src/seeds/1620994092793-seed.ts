...
export class seed1620994092793 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    postsList.forEach(async ({ title, date, description, contents }) => {
      await queryRunner.query(
        'INSERT INTO posts (title, date, description, contents) VALUES (?, ?, ?, ?)',
    ...
    postTagList.forEach(async ({ postsId, tagsId }) => {
      await queryRunner.query('INSERT INTO posts_tags_tags(postsId, tagsId) VALUES (?, ?)', [
        postsId,
        tagsId,
      ]);
    });
